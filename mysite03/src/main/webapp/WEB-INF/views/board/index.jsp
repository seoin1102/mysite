		...
		<div id="content">
			<div id="board">
				...
				<table class="tbl-ex">
					...
					<c:if test='${end >=5}'>
						<c:choose>
									...
									<c:if test='${fn:contains(vo.title,param.kwd)}'>
										<tr>
											...
											<c:if test='${vo.depth != 1}'>
												<td style="text-align:left; padding-left:${(vo.depth-1)*5}0px">
							...
							<c:otherwise>
								<c:forEach items='${list}' begin='${end-5}' end='${end-1}'
									...
									<tr>
										...
										<c:if test='${vo.depth == 1}'>
											<td style="text-align:left; padding-left:${(vo.depth-1)*10}0px">
											<a href="${pageContext.request.contextPath }/board/view/${vo.no}">${vo.title}</a></td>
										</c:if>
										<td>${vo.userName}</td>
										<td>${vo.hit}</td>
				...
				<div class="pager">
					<ul>
						<c:if test='${end-5>0}'>
							<li><a href="${pageContext.request.contextPath }/board/page/${end-5}">◀</a></li>
						</c:if>
						<c:forEach begin='${end/5}' end='${end/5+4}' step='1' var='i'>
							...
							<c:choose>
								...
								<c:when test='${count/5+1>i}'>
									<li><a href="${pageContext.request.contextPath }/board/page/${i*5}">${i}</a></li>
								</c:when>
								<c:otherwise>
									<li>${i}</li>
								</c:otherwise>
